...
###
 ...
 # @FilePath: /llava_med/LLaVA-Med/llava/run/llava_med_scripts/scripts_A100/2025-01-01/llava_mistral_clip_A100_v1_mlp4_lr2e5_layer2_tmlp1_2A100/train_merge_lora.sh
 # @Description: 
### 
...
deepspeed train/train_mem.py \
    ...
    --bf16 True \
    --mis_mlp_lr 2e-5 \
    --output_dir ./checkpoints/llava-lora-new-clip-A100-version4_1_1 \
    --num_train_epochs 1 \
...
python -m llava.run.train.merge_lora_weights \
    --model-path ./checkpoints/llava-lora-new-clip-A100-version4_1_1 \
    --model-base /mnt/nlp-ali/usr/huangwenxuan/home/official_llava_med/llava-med-v1.5-mistral-7b \
    --save-model-path ./checkpoints/llava_mistral_new_clip_a100_version4_1_1 \
    --ncls_count 4 \
    --hidden_dim 1024 \
    --output_dim 4096 \
    --mlp_type 4 \
