...
###
 ...
 # @FilePath: /llava_med/LLaVA-Med/llava/run/llava_med_scripts/scripts_A100/2025-02-06/llava_mistral_clip_A100_v5_mlp5_mlplr2e4_out4096_layer2_2epoch_2A100/train_merge_lora.sh
 # @Description: 
### 
...
deepspeed train/train_mem.py \
    ...
    --bf16 True \
    --mis_mlp_lr 2e-4 \
    --output_dir ./checkpoints/llava-lora-new-clip-A100-version5_2_6 \
    --num_train_epochs 2 \
...
python -m llava.run.train.merge_lora_weights \
    --model-path ./checkpoints/llava-lora-new-clip-A100-version5_2_6 \
    --model-base /mnt/nlp-ali/usr/huangwenxuan/home/official_llava_med/llava-med-v1.5-mistral-7b \
    --save-model-path ./checkpoints/llava_mistral_new_clip_a100_version5_2_6 \
    --Imgcls_count 4 \
    --Txtcls_count 8 \
    --hidden_dim 1024 \
    --output_dim 4096 \
