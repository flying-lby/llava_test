...
###
 ...
 # @FilePath: /llava_med/LLaVA-Med/llava/run/llava_med_scripts/scripts_A100/2025-02-01/llava_mistral_clip_A100_v3_img4_txt16_lr2e5_layer2_2epoch_2A100/train_merge_lora.sh
 # @Description: 
### 
...
deepspeed train/train_mem.py \
    ...
    --bf16 True \
    --mis_mlp_lr 2e-5 \
    --output_dir ./checkpoints/llava-lora-new-clip-A100-version3_2_1 \
    --num_train_epochs 2 \
...
python -m llava.run.train.merge_lora_weights \
    --model-path ./checkpoints/llava-lora-new-clip-A100-version3_2_1 \
    --model-base /mnt/nlp-ali/usr/huangwenxuan/home/official_llava_med/llava-med-v1.5-mistral-7b \
    --save-model-path ./checkpoints/llava_mistral_new_clip_a100_version3_2_1 \
    --Imgcls_count 4 \
    --Txtcls_count 16 \
    --hidden_dim 1024 \
    --output_dim 4096 \
