...
###
 ...
 # @FilePath: /llava_med/LLaVA-Med/llava/run/llava_med_scripts/ex_scripts_R4090/test_wosft_llava_llavamed_chestxray_classify_4090/llava7b_pipeline.sh
...
deepspeed train/train_mem.py \
    ...
    --model_name_or_path /srv/lby/llava_mistral_7b_official \
    --version v1 \
    ...
    --bf16 True \
    --mis_mlp_lr 5e-5 \
    --output_dir srv/lby/llava_med/checkpoints/llava7b-lora-sft-v1 \
    --num_train_epochs 1 \
...
python -m llava.run.train.merge_lora_weights \
    --model-path srv/lby/llava_med/checkpoints/llava7b-lora-sft-v1 \
    --model-base /srv/lby/llava_mistral_7b_official \
    --save-model-path /srv/lby/llava_med/checkpoints/llava7b_mistral_sft_v1 
...
python -m llava.run.eval.origin_eval_classify_chestxray \
    --model-path /srv/lby/llava_med/checkpoints/llava7b_mistral_sft_v1 \
    --output-path ./data/chest_xray/Chest-X-ray_llava_origin_val_ans.jsonl \
    --class_path ./data/chest_xray/Chest-X-ray_classes.json \
    --result-file ./result/experiments/Ex_R4090/llava7b_sft_v1_Chest_Xray_classify_clip.txt \
...
python -m llava.run.eval.origin_eval_classify_chestxray \
    ...
    --output-path ./data/chest_xray/Chest-X-ray_llava_origin_val_ans.jsonl \
    --class_path ./data/chest_xray/Chest-X-ray_classes.json \
    --result-file ./result/experiments/Ex_R4090/llava7b_sft_v1_Chest_Xray_classify_origin.txt \
    --question-file ./data/chest_xray/Chest-X-ray_llava_origin_val.jsonl \
    --inference origin \
    --image-folder "/srv/lby" \
    --conv-mode vicuna_v1 
