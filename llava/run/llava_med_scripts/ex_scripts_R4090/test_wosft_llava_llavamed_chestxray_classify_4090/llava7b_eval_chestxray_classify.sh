###
 ...
 # @FilePath: /llava_med/LLaVA-Med/llava/run/llava_med_scripts/ex_scripts_R4090/test_wosft_llava_llavamed_chestxray_classify_4090/llava7b_eval_chestxray_classify.sh
 # @Description: 
### 
...
python -m llava.run.eval.origin_eval_classify \
    --model-path /srv/lby/llava_mistral_7b_official \
    --output-path ./data/chestxray/llava7b/Chest-X-ray_llava_origin_val_ans.jsonl \
    --dataset "chestxray" \
    --chexpert-subset "False" \
    --result-file ./result/experiments/Ex_R4090/llava7b/Chest-X-ray_classify_origin.txt \
    --inference origin \
    --image-folder "/srv/lby/" \
    --conv-mode vicuna_v1 
