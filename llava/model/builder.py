...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda:0", use_flash_attn=False):
    ...
    #         model = LlavaMistralForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, tokenizer=tokenizer, config=lora_cfg_pretrained, **kwargs)
    ...
    # # 手动检查embed_tokens的权重
    # print(f"embed_tokens weight before any operations: {model.model.embed_tokens.weight.sum()}")

    # # 手动初始化embed_tokens权重，防止为零
    # if model.model.embed_tokens.weight.sum() == 0:
    #     print("Reinitializing embed_tokens weight...")
    #     token_num, tokem_dim = model.model.embed_tokens.weight.shape
    #     model.model.embed_tokens.weight = torch.nn.Parameter(torch.randn(token_num, tokem_dim, device=model.device, dtype=model.dtype))

    # # 再次检查embed_tokens的权重
    # print(f"embed_tokens weight after reinitializing: {model.model.embed_tokens.weight.sum()}")
