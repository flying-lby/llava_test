...
#     best_threshold = 0.0
#     best_f1 = 0.0
#     thresholds = [i * 0.01 for i in range(100)]  # 0到1之间的阈值
    
#     for threshold in thresholds:
#         preds_bin = (preds > threshold).float()
#         f1 = f1_score(labels.cpu(), preds_bin.cpu(), average='weighted', zero_division=1)
#         if f1 > best_f1:
#             best_f1 = f1
...
def main(dataset, model_name, probe, split, feature_type, n_epochs):
    ...
    #         test_f1_per_class = []
    #         for i in range(len(classes)):
    #             print(f"Class {classes[i]} - Positive predictions (Train): {(train_preds_bin[:, i] == 1).sum().item()}")
    #             print(f"Class {classes[i]} - Positive predictions (Test): {(test_preds_bin[:, i] == 1).sum().item()}")
    #             train_f1_per_class.append(f1_score(train_labels[:, i].cpu(), train_preds_bin[:, i].cpu(), average='macro', zero_division=1))
    #             test_f1_per_class.append(f1_score(test_labels[:, i].cpu(), test_preds_bin[:, i].cpu(), average='macro', zero_division=1))

    #         # 计算每个标签的 AUC
    #         train_auc_per_class = []
    #         test_auc_per_class = []
    #         for i in range(len(classes)):
    #             train_auc_per_class.append(roc_auc_score(train_labels[:, i].cpu(), train_preds[:, i].cpu()))
    #             test_auc_per_class.append(roc_auc_score(test_labels[:, i].cpu(), test_preds[:, i].cpu()))

    #         # 计算平均准确率、F1和AUC
    #         mean_train_acc = torch.tensor(train_acc_per_class).mean().item()
    #         mean_test_acc = torch.tensor(test_acc_per_class).mean().item()
    #         mean_train_f1 = torch.tensor(train_f1_per_class).mean().item()
    #         mean_test_f1 = torch.tensor(test_f1_per_class).mean().item()
    #         mean_train_auc = torch.tensor(train_auc_per_class).mean().item()
    #         mean_test_auc = torch.tensor(test_auc_per_class).mean().item()

    #         # 记录结果
    #         accs.append((mean_train_acc, mean_test_acc))
    #         print(f"Epoch {epoch + 1}: Train Acc: {mean_train_acc:.4f}, Test Acc: {mean_test_acc:.4f}")
    #         print(f"Epoch {epoch + 1}: Train F1: {mean_train_f1:.4f}, Test F1: {mean_test_f1:.4f}")
    #         print(f"Epoch {epoch + 1}: Train AUC: {mean_train_auc:.4f}, Test AUC: {mean_test_auc:.4f}")
    ...
    # # 保存准确率图和模型
    # output_prefix = f"probe_outputs/new_{dataset}_{model_name}_{probe}_{split}_{feature_type}"
    # plt.savefig(f"{output_prefix}.png")
    # torch.save([accs, model.state_dict()], f"{output_prefix}.pt")


@click.command()
@click.option("--model_id", default="/srv/lby/llava_med/checkpoints/llava-mistral_ft2")
...
@click.option("--probe", default="mlp")
@click.option("--split", default="test")
@click.option("--feature_type", default="avg")
